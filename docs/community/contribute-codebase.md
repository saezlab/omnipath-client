...
dependencies needed for running omnipath-client (`[tool.poetry.dependencies]` with the
...
developing omnipath-client using test-driven development. Please make sure that you
...
If you are brand new to omnipath-client or open-source development, we recommend
...
omnipath-client is hosted on GitHub, and to contribute, you will need to sign up for a
...
### Create a fork of omnipath-client

You will need your own fork of omnipath-clientin order to eventually open a Pull
Request. Go to the omnipath-client project page and hit the Fork button. Please
...
```bash
git clone https://github.com/your-user-name/omnipath-client.git
cd omnipath-client
git remote add upstream https://github.com/omnipath-client/omnipath-client.git
git fetch upstream
```

This creates the directory `omnipath-client` and connects your repository to the
upstream (main project) *omnipath-client* repository. They have the same name, but
your local repository and fork are separate from the upstream repository.

### Creating a feature branch

Your local `main` branch should always reflect the current state of omnipath-client
repository. First ensure it's up-to-date with the main omnipath-client repository.
...
to create a branch called `my-new-feature-for-omnipath-client`

```bash
git checkout -b my-new-feature-for-omnipath-client
```

This changes your working branch from `main` to the
`my-new-feature-for-omnipath-client` branch. Keep any changes in this branch specific
to one bug or feature so it is clear what the branch brings to *omnipath-client*. You
...
When making changes, follow these omnipath-client-specific guidelines:
...
Now your code is on GitHub, but it is not yet a part of the omnipath-client project.
...
1. Write a descriptive title that includes prefixes. omnipath-client uses a convention for title prefixes, most commonly, `feat:` for features, `fix:` for bug fixes, and `refactor:` for refactoring.
...
branch with changes in the omnipath-client `main` branch, run:

```shell

    git checkout my-new-feature-for-omnipath-client
    git fetch upstream
...
```shell
    git push origin my-new-feature-for-omnipath-client
```
